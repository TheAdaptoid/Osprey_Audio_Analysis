...
class Distance_Functions:
    def _Verify_Dimensionality(
        self, aVector: list[int | float], bVector: list[int | float]
    ) -> None:
        """
        ...
        if len(aVector) != len(bVector):
            raise ValueError(
                f"Vectors must have the same dimensionality: {len(aVector)} != {len(bVector)}"
            )

    def Euclidean_Between(
        self, aVector: list[int | float], bVector: list[int | float]
    ) -> float:
        """
...
class Cluster:
    ...
    def _Median_Vector(
        self, vectors: list[list[int | float]], distanceFunction: Callable
    ) -> list[int | float]:
        """
    ...
    def Recalculate_Centroid(
        self, centroidStrategy: str, distanceFunction: Callable
    ) -> bool:
        """
    ...
    def __dict__(self) -> dict:
        ...
        return {"name": self.name, "centroid": self.centroid, "vectors": self.vectors}
...
class K_Clusters:
    ...
    def _Initialize_Clusters(self) -> list[Cluster]:
        ...
        clusters: list[Cluster] = []
        randomVectors: list[list[int | float]] = choices(
            self.trainingMatrix, k=self.kGroups
        )

        for i in range(self.kGroups):
            clusters.append(Cluster(name=f"cluster_{i}", centroid=randomVectors[i]))
    ...
    def _Get_Nearest_Cluster(self, vector: list[int | float]) -> Cluster:
        ...
        indexOfNearestCluster: int = min(
            centroidSeparations, key=centroidSeparations.get
        )
    ...
    def _Calculate_Silhouette_Scores(self) -> None:
        for cIndex, cluster in enumerate(self.clusters):
            ...
            for vector in cluster.vectors:
                ...
                vectorToCentroid: float = self._Get_Distance_Function()(
                ...
                otherCentroids: list[list[int | float]] = [
                    cluster.centroid
                    for cJndex, cluster in enumerate(self.clusters)
                    if cIndex != cJndex
                ]
                vectorToNeighbor: float = min(
                    [
                        self._Get_Distance_Function()(vector, centroid)
                        for centroid in otherCentroids
                    ]
                )

                # Calculate the silhouette score
                silhouetteScore: float = (vectorToNeighbor - vectorToCentroid) / max(
                    vectorToNeighbor, vectorToCentroid
                )
        ...
        self.silhouetteScore = round(
            number=sum([cluster.silhouetteScore for cluster in self.clusters])
            / len(self.clusters),
            ndigits=4,
        )
